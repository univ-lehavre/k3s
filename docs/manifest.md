...
Les manifestes pilot decrivent l'etat attendu d'une machine. Ils peuvent etre versionnes dans un depot public tant qu'ils ne contiennent pas d'informations de connexion reelles.
...
```bash
uv run pilot validate examples/single-server.yaml --inventory inventory.local.yaml
```
